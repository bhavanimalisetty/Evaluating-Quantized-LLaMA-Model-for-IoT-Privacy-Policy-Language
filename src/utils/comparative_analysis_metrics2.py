...
class RougeMetric:
    def __init__(self) -> None:
        ...
        for dataframe_name in ["df_q4", "df_q5", "df_q8", "df_base_model"]:
    ...
    def getRougeMetric(self, df_q4: pd.DataFrame, df_q5: pd.DataFrame, df_q8: pd.DataFrame, df_base_model: pd.DataFrame) -> None:
        ...
        Returns:
            None
        """
        dataframes = {"df_q4": df_q4, "df_q5": df_q5, "df_q8": df_q8, "df_base_model": df_base_model}
        ...
        for rouge_type in self.rouge_scores['df_q4']:            
            ...
            df_base_data = self.rouge_scores['df_base_model'][rouge_type]['data']
            ...
            # Create box plots
            bplot = plt.boxplot([df_q4_data, df_q5_data, df_q8_data, df_base_data], labels=['4 bit', '5 bit', '8 bit',"Base Model"], patch_artist=True, boxprops=boxprops, medianprops=medianprops, flierprops=flierprops)
            ...
            plt.title(f'{rouge_type} Score Variability Across Quantized and Base LLaMA-2 Models')
...
class BertMetric:
    ...
    def getBertMetric(self, df_q4: pd.DataFrame, df_q5: pd.DataFrame, df_q8: pd.DataFrame, df_base_model: pd.DataFrame) -> None:
        ...
        for metric in self.BERT_METRICS:
            ...
            df_base_data = self.bert_scores["df_base_model"][metric]["data"]
            boxprops = dict(facecolor = "none", edgecolor = 'black')
            medianprops = dict(color='black')
            flierprops = dict(marker = 'o', markerfacecolor = "#F55252", markersize = 8, linestyle = 'none')
            ...
            # Set plot title and labels
            plt.title(f'{metric.capitalize()} Score Variability Across Quantized and Base LLaMA-2 Models')
            plt.xlabel("LLaMA-2 Model Variants")
...
class WordEmbeddingsModel:    
    ...
    def getWordEmbeddings(self, df_4bit: pd.DataFrame, df_5bit: pd.DataFrame, df_8bit: pd.DataFrame, df_base_model: pd.DataFrame):
        ...
        """
        data = []
        dfs = [df_4bit, df_5bit, df_8bit, df_base_model]
...
class Metrics:
    def getAllMetrics(self, df_q4: pd.DataFrame, df_q5: pd.DataFrame, df_q8: pd.DataFrame, df_base_model: pd.DataFrame) -> None:
        """ This function is used to generate metrics for dataframes containing reference and generated text using 
            Rouge, Bert, Roberta, PyTorch, Word2Vec, Glove, and FastText.
        ...
        rougeMetric.getRougeMetric(df_q4, df_q5, df_q8, df_base_model)
        bertMetric = BertMetric()
        bertMetric.getBertMetric(df_q4, df_q5, df_q8,df_base_model)
        # robertaMetric = RobertaMetric()
        # robertaMetric.getRobertaMetrics(df_q4, df_q5, df_q8)
        # pyTorchMetric = PyTorchMetric()
        # pyTorchMetric.getPyTorch(df_q4, df_q5, df_q8)
        ...
        for wordEmbeddingModelName in WordEmbeddingModels:
            ...
            wordEmbeddingModel.getWordEmbeddings(df_q4, df_q5, df_q8, df_base_model)
